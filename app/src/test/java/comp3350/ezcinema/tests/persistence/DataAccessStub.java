...
public class DataAccessStub implements DataAccess {
    ...
    @Override
    public void getTicketsSequential(ArrayList<Ticket> emptyTickets) {

        emptyTickets.addAll(tickets);
    ...
    public int insertTicket(String movieName, String theaterName, String showTime, int row, int col) {
        ...
        //just used to fill the array for our MT, no issue because its not used in checkStatus
        ArrayList<String> temp = new ArrayList<>();
        temp.add("00:00");
        temp.add("11:11");
        temp.add("22:22");

        if(movieName != null && theaterName != null && showTime != null && row > -1 && row < 5 && col > -1 && col < 5) {

            MT mt = new MT(movieName, theaterName, temp);

            //check if they exist
            if(checkStatus(mt, showTime, row, col) == 0) {
                ticket = new Ticket(movieName, theaterName, showTime, row, col);

                updateStatus(mt, showTime, row, col);
                tickets.add(ticket);
                result = 1;
            }
        }
